...
const OcrFieldsEditor = ({ extractedFields, onReanalyze, isLoading }: OcrFieldsEditorProps) => {
  ...
  const handleChange = (testType: string, raw: string) => {
    ...
    if (raw && isNaN(parseFloat(raw))) {
      setFieldErrors(prev => ({ ...prev, [testType]: 'Enter a valid number' }));
    } else {
      setFieldErrors(prev => {
        const next = { ...prev };
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="px-5 py-4 bg-gradient-to-r from-slate-50 to-blue-50 border-b border-slate-100 flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-slate-800">OCR Detected Values — Please Verify</p>
          <p className="text-xs text-slate-500 mt-0.5">
            Review what the AI read from your report. Edit if needed, then confirm.
          </p>
        </div>
      </div>
      ...
      {hasLowConfidence && (
        <div className="flex items-start gap-2.5 px-5 py-3 bg-amber-50 border-b border-amber-100">
          ...
          <p className="text-xs text-amber-700">
            Some values have low OCR confidence. Compare with your original report before confirming.
          </p>
        </div>
      )}
            ...
            <div key={testType} className="px-5 py-4">
              ...
              {isLow && !error && (
                <p className="mt-1.5 text-xs text-amber-600">
                  Low confidence — verify against your printed report.
                </p>
              )}
            </div>
          );
      ...
      <div className="px-5 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between gap-3">
        <p className="text-xs text-slate-400">
          Editing a value will re-run ADA classification with your correction.
        </p>
        <button
